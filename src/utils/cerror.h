...
#define CERROR_memory_allocation                                               \
  ((CError){ .code = 256, .msg = "c: Memory allocation/reallocation error" })
#define CERROR_str_exccedded_len                                               \
  ((CError){ .code = 257, .msg = "c: String excedded the length" })
#define CERROR_out_is_null                                                     \
  ((CError){ .code = 258, .msg = "c: The out pointer is null" })
#define CERROR_no_such_source                                                  \
  ((CError){ .code = 259, .msg = "c: Source doesn't exists" })
...
#define CERROR_invalid_target_type                                             \
  ((CError){ .code = 262, .msg = "c: Invalid target type" })
