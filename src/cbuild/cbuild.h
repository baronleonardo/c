...
// target create
__C_DLL__ CError cbuild_object_create(CBuild*    self,
                                      char const name[],
                                      size_t     name_len,
                                      char const base_path[],
                                      size_t     base_path_len,
                                      CTarget*   out_target);
__C_DLL__ CError cbuild_static_lib_create(CBuild*    self,
                                          char const name[],
                                          size_t     name_len,
                                          char const base_path[],
                                          size_t     base_path_len,
                                          CTarget*   out_target);
__C_DLL__ CError cbuild_shared_lib_create(CBuild*    self,
                                          char const name[],
                                          size_t     name_len,
                                          char const base_path[],
                                          size_t     base_path_len,
                                          CTarget*   out_target);
__C_DLL__ CError cbuild_exe_create(CBuild*    self,
                                   char const name[],
                                   size_t     name_len,
                                   char const base_path[],
                                   size_t     base_path_len,
...
__C_DLL__ CError cbuild_target_add_source(CBuild*    self,
                                          CTarget*   target,
                                          char const source_path[],
                                          size_t     source_path_len);

__C_DLL__ CError cbuild_target_set_include_path(CBuild*    self,
                                                CTarget*   target,
                                                char const include_path[],
                                                size_t     include_path_len);

__C_DLL__ CError cbuild_target_add_include_path_flag(CBuild*    self,
                                                     CTarget*   target,
                                                     char const include_path[],
                                                     size_t include_path_len);

__C_DLL__ CError cbuild_target_depends_on(CBuild*         self,
                                          CTarget*        target,
                                          CTarget*        depend_on,
                                          CTargetProperty resource);

__C_DLL__ CError cbuild_target_add_compile_flag(CBuild*    self,
                                                CTarget*   target,
                                                char const flag[],
                                                size_t     flag_len);

__C_DLL__ CError cbuild_target_add_link_flag(CBuild*    self,
                                             CTarget*   target,
                                             char const flag[],
                                             size_t     flag_len);

__C_DLL__ CError cbuild_target_get(CBuild*    self,
                                   char const target_name[],
                                   size_t     target_name_len,
                                   CTarget*   out_target);

__C_DLL__ CError cbuild_depends_on(CBuild*    self,
