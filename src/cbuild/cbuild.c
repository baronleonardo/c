              ...
              CBuild* out_cbuild)
{
  ...
  *out_cbuild = (CBuild){ impl };
  ...
  c_defer_check(str_err.code == 0,
                ...
                err = CERROR_internal_error(str_err.desc));
  str_err =
    c_str_set_capacity(&out_cbuild->impl->base_path, c_fs_path_get_max_len());
  c_defer_check(str_err.code == 0,
                c_str_destroy,
                &out_cbuild->impl->base_path,
                err = CERROR_internal_error(str_err.desc));
  c_fs_error_t fs_err =
    c_fs_path_to_absolute(base_path,
                          base_path_len,
                          out_cbuild->impl->base_path.data,
                          out_cbuild->impl->base_path.capacity,
                          &out_cbuild->impl->base_path.len);
  c_defer_check(fs_err.code == 0,
                c_str_destroy,
                &out_cbuild->impl->base_path,
...
cbuild_object_create(CBuild* self,
                     ...
                     CTarget* out_target)
{
  return cbuild_target_create(self,
                              name,
                              name_len,
                              base_path,
                              base_path_len,
                         ...
                         size_t source_path_len)
{
  ...
  if (!is_absolute) {
    ...
    c_fs_error_t fs_err = c_fs_path_append(target_path.data,
                                           target_path.len,
                               ...
                               size_t include_path_len)
{
  ...
  c_str_error_t str_err = c_str_replace_at(&target->impl->base_dir,
                                           0,
                                    ...
                                    size_t include_path_len)
{
  ...
  c_str_error_t str_err = c_str_format(&target->impl->cflags,
                         ...
                         CTargetProperty property)
{
  ...
  if ((property & CTARGET_PROPERTY_objects) == CTARGET_PROPERTY_objects) {
    ...
    if (strcmp(target->impl->base_dir.data, depend_on->impl->base_dir.data) !=
      ...
      str_err = c_str_format(&build_path,
                             0,
                             C_STR_INV("%s%c%s"),
  ...
  if (((property & CTARGET_PROPERTY_library) == CTARGET_PROPERTY_library) ||
    ...
    char separator = c_fs_path_get_separator();
    str_err = c_str_format(&install_path,
                           0,
                           C_STR_INV("%s%c%s"),
                           depend_on->impl->base_dir.data,
                           separator,
    ...
    // -L<library path>
    str_err = c_str_format(&target->impl->lflags,
    ...
    // -l<library>
    str_err = c_str_format(&target->impl->link_with,
    ...
    str_err = c_str_append_with_cstr(
      &target->impl->link_with, C_STR2(default_builder->extension.lib_static));
    c_defer_check(
      str_err.code == 0, NULL, NULL, err = CERROR_internal_error(str_err.desc));
#endif
  }
  ...
  if ((property & CTARGET_PROPERTY_cflags) == CTARGET_PROPERTY_cflags) {
    str_err = c_str_format(&target->impl->cflags,
                           target->impl->cflags.len,
                           C_STR_INV(" %s"),
  ...
  if ((property & CTARGET_PROPERTY_include_path) ==
    ...
    err = cbuild_target_add_include_path_flag(self,
                                              target,
                                              depend_on->impl->base_dir.data,
                  ...
                  CBuild* out_other_cbuild)
{
  ...
  err = cbuild_create(self->impl->btype,
                      other_cbuild_path,
...
cbuild_configure(CBuild* self)
{
  ...
  fs_err = c_fs_path_append(cbuild_dll_path.data,
                            cbuild_dll_path.len,
  ...
  fs_err = c_fs_path_to_absolute(cbuild_dll_path.data,
                                 cbuild_dll_path.len,
  ...
  c_dl_error_t dl_err =
    c_dl_loader_create(cbuild_dll_path.data, cbuild_dll_path.len, &dll_loader);
  c_defer_err(dl_err.code == 0,
              c_dl_loader_destroy,
              &dll_loader,
              err = CERROR_internal_error(dl_err.desc));

  CError (*build_fn)(CBuild*);
  dl_err = c_dl_loader_get(&dll_loader,
...
cbuild_build(CBuild* self)
{
  ...
  for (size_t i = 0; i < self->impl->targets.len; ++i) {
    err =
      cbuild_target_build(self, ((CTargetImpl**)self->impl->targets.data)[i]);
    c_defer_check(err.code == 0, NULL, NULL, NULL);
  }
                     ...
                     CTarget* out_target)
{
  ...
  *out_target = (CTarget){ 0 };
  ...
  // base dir
  str_err =
    c_str_set_capacity(&out_target->impl->base_dir, c_fs_path_get_max_len());
  c_defer_check(
    str_err.code == 0, NULL, NULL, err = CERROR_internal_error(str_err.desc));
  bool is_absolute = false;
  c_fs_error_t fs_err =
  ...
  if (!is_absolute) {
    fs_err = c_fs_path_to_absolute(base_path,
                                   base_path_len,
    ...
    str_err = c_str_replace_at(&out_target->impl->base_dir,
                               0,
                               out_target->impl->base_dir.len,
                               base_path,
                               base_path_len);
...
cbuild_target_compile(CBuild* self, CTargetImpl* target)
{
  ...
  c_defer_err(str_err.code == 0,
              ...
              &obj_output,
              err = CERROR_internal_error(str_err.desc));
  str_err = c_str_format(&obj_output,
                         0,
                         C_STR_INV("%s%s%c%s"),
  ...
  c_defer_err(str_err.code == 0,
              ...
              &pdb_output,
              err = CERROR_internal_error(str_err.desc));
  str_err = c_str_format(&pdb_output,
                         0,
                         C_STR_INV("%s%s%c%s"),
...
cbuild_target_link(CBuild* self, CTargetImpl* target)
{
  ...
  if (target->ttype == CTARGET_TYPE_static) {
    ...
    if (strcmp("ar",
               &self->impl->cmds.static_lib_creator
                  .data[self->impl->cmds.static_lib_creator.len - 2]) == 0) {
      flag_output = "";
    }
  ...
  c_fs_error_t fs_err =
    c_fs_foreach(target->build_path.data,
                                 ...
                                 CStr* build_fn_name)
{
  ...
  c_defer_check(str_err.code == 0,
                ...
                err = CERROR_internal_error(str_err.desc));
  err =
    internal_find_build_function_name(C_STR("./build.c"), &build_function_name);
  c_defer_check(err.code == 0, NULL, NULL, NULL);
  ...
  c_defer_check(
    fs_err.code == 0, NULL, NULL, err = CERROR_internal_error(fs_err.desc));
  fs_err =
    c_fs_path_get_parent(cur_exe_dir.data, cur_exe_dir.len, &cur_exe_dir.len);
  c_defer_check(
    fs_err.code == 0, NULL, NULL, err = CERROR_internal_error(fs_err.desc));
  ...
  err =
    cbuild_target_add_link_flag(self, &build_target, lflags.data, lflags.len);
  c_defer_check(err.code == 0, NULL, NULL, NULL);
#endif
